...
simulation_state = {
    ...
    'resources': ['Free', 'Free', 'Free', 'Free','Free'],
    'waiting_queue': [],
    'log': ['System initialized.'],
    'utilization': 0,
}

resource_labels = ["DB Connection", "Media Service", "Notification Service", "Message Queue","update feeds"]
...
def create_user(name):
    needed_resources = random.sample(range(5), random.randint(1, 5))
    return {
...
def detect_deadlock_with_nx():
    ...
    # Add edges for waiting relationships based on resource requests
    for proc, info in user_info.items():
        # A process is only waiting if it's not done and not yet running
        if info['current_resource_index'] < len(info['resources_needed']):
            ...
            # If the resource is held by another user, add a waiting edge
            if holder != 'Free' and holder != proc and holder in user_info:
                wait_for_graph.add_edge(proc, holder)

    # Find cycles to detect deadlocks
    ...
    while True:
        with simulation_lock:
            ...
            for i, proc in enumerate(simulation_state['handler_frames']):
                ...
                # If a process is finished, clean it up
                if info['state'] == 'finished':
                    ...
                    for r in list(info['held_resources']):
                        if simulation_state['resources'][r] == proc:
                            ...
                            log_event(f"{proc} released {resource_labels[r]}")
                    del user_info[proc]
                    next_handler_frames[i] = '(Empty)'
                    continue

                # If the user is running, try to allocate the next resource
                if info['state'] == 'running':
                    # Check if the process is done with the current resource's timer
                    if info['timer'] > 0:
                        info['timer'] -= 1
                        continue
                    ...
                    if cr_idx < len(info['resources_needed']):
                        ...
                        if simulation_state['resources'][r_needed] == 'Free':
                            ...
                            log_event(f"{proc} allocated {resource_labels[r_needed]}")
                            info['timer'] = random.randint(3, 8)
                            
                            # If this was the last resource, the user is finished
                            if info['current_resource_index'] == len(info['resources_needed']):
                                info['state'] = 'finished'
                        else:
                            # Resource is not free, block the user
                            info['state'] = 'blocked'
            ...
            if deadlocks:
                ...
                # For each detected cycle, resolve it by killing one user
                for cycle in deadlocks:
                    ...
                    if proc_to_kill in user_info:
                        ...
                        # Remove from all queues and handler frames
...
def start_simulation():
    ...
    with simulation_lock:
        if not simulation_state['running']:
            ...
            # This check prevents a race condition if start is called twice
        ...
        for proc, info in user_info.items():
            if info['current_resource_index'] < len(info['resources_needed']):
                ...
                if holder != 'Free' and holder in user_info and holder != proc:
                    edges.append({'from': proc, 'to': holder, 'arrows': 'to', 'label': f"waits for {resource_labels[waiting_resource_index]}"})
